...
function AdminDashBord() {
  ...
  const [userRoleShow, setUserRoleShow] = useState([]); 
    ...
    {headers:{
      accesToken: localStorage.getItem("accesToken")
         ...
         .then((response)=>{
          ...
          setUserRoleShow(response.data.listOfUsers.map((user)=>{
            return user.role
          }))
  ...
  const deleteComment = (data) =>{
    ...
    const updateRole = (userInfo) => {
      ...
      userRoleShow = userInfo.role;
      let newRole = ""
        if(role === "0"){
           newRole = true; 
           setUserRoleShow({Admin:true});        
        }else{
          newRole = false
        }
