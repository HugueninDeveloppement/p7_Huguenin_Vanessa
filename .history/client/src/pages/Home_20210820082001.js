        ...
        return (
          <div key={key} className="post-card" >
            ...
            <div className="footer-post-card">
              <div >            
                  <Link to={`/profile/${value.UserId}`} style={{ textDecoration: 'none'}}>{value.userPseudo}</Link>             
              </div>
              <div className="like-choose">
                { (listOfLikeInPost.includes(value.id)) ?               
                  <ThumbUpAltIcon className="ThumbUpAltIcon" onClick={()=> likePost(value.id)}/> : <ThumbUpAltOutlinedIcon className="ThumbUpAltOutlinedIcon" onClick={()=> likePost(value.id)}/>              
                }            
              <span>{value.Likes.length}</span>
              </div>
              <div>
              { (listOfPostInAlert.includes(value.id)) ?    
                 <ReportProblemIcon className="ReportProblemIcon" onClick={()=> signalPost(value.id)} titleAccess="Vous avez déjà signaler ce problème"/>:
