...
function Home() {
  ...
  const signalPost = (postId) => {
    Axios.post("http://localhost:3006/api/alert/post",
     ...
     .then((response)=>{
      setListOfPostInAlert(listOfPosts.map((post)=> {         
         if (post.id === postId){
           if(response.data.liked){
            return{...post, Likes: [...post.Likes, 0]}
          }else {
    ...
    listOfPosts.map((value, key) => {
        return (
          <div key={key} className="post" >
            ...
            <div>
              <ReportProblemOutlinedIcon titleAccess="signaler un probléme avec ce post" />
              <WarningOutlinedIcon titleAccess="Vous avez déjà signaler ce problème"/>
            </div>
          </div>
        )
      })}
