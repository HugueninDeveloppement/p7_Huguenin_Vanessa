...
function Home() {
  ...
  const signalPost = (PostId,UserId) => {
        ...
        return (
          <div key={key} className="post" >
            ...
            { (listOfPostInAlert.includes(value.id)) ?    
              <ReportProblemOutlinedIcon onClick={()=> signalPost(value.id,authState.UserId)} titleAccess="signaler un probléme avec ce post" /> :
              <WarningOutlinedIcon onClick={()=> signalPost(value.id)} titleAccess="Vous avez déjà signaler ce problème"/>
            }
            </div>
          </div>
        )
