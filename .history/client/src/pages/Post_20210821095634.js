    ...
    return (
        <div className="postPage">
            ...
            <div  className="head-postContainer">
                ...
                <div className="head-postContainer-userPseudo">
                    <div>
                     <h1> {postObject.userPseudo}</h1>
                     </div>
                {authState.userPseudo === postObject.userPseudo && 
                     <div>
                     <HighlightOffIcon className="HighlightOffIcon" onClick={()=>{
                        deletePost(postObject.id)
                        }}/>
                    </div>
                }
                <div>
                {Date(postObject.createdAt).slice(3,15)} 
                </div>
                </div>
            </div>
            <div className="commentContainer">
                <div className="commentContainerinput">
                    <input type="text" placeholder="commentaire...  (250 caractere max)" autoComplete="off" value={newComment} onChange={(e) => {
            ...
            <div className="comment-contain">
                    ...
                    {comments.map((comment , key) => {
                        return (
                            ...
                            <div  key= {key} className="onecomment">
                                <div className="comment-contain-text">{comment.commentText}</div>
                                <div className="comment-contain-footer">
                                    ...
                                    { (listOfAlertInComment.includes(comment.id)) ? 
                                        <ReportProblemIcon className="ReportProblemIcon" onClick={()=> signalComment(comment.id)} titleAccess="Vous avez déjà signalé ce commentaire"/>:
                                        <ReportProblemOutlinedIcon className="ReportProblemOutlinedIcon" onClick={()=> signalComment(comment.id)} titleAccess="Signaler un probléme avec ce commentaire"/> 
