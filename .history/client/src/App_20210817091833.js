...
function App() {
  const [authState, setAuthState] = useState({id:0,userPseudo:"", userRole:"", status:false}); 
  ...
  useEffect(()=>{
    ...
    Axios.get("http://localhost:3006/api/users/checkToken" , {
      ...
      if(response.data.error){
        ...
        setAuthState({userPseudo:response.data.userPseudo, id:response.data.id, userRole:response.data.role, status:true})
  ...
  const logout = () =>{ 
    ...
    setAuthState({userPseudo:"", id:0, userRole:"", status:false});
  }
