...
Router.get("/userAllInfo/:id", validateToken ,async (req, res)=> {
    ...
    const userAllInfo = await Users.findAll({order:[['id','DESC']],include: [Posts,Comments]});
...
Router.get("/userControl", validateToken ,async (req, res)=> {
    ...
    const listOfUsers = await Users.findByPk(id,{order:[['id','DESC']],attributes: {exclude: ["userRole"]}})
