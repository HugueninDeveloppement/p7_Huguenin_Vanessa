...
Router.get("/userAllInfo/:id", validateToken ,async (req, res)=> {
    ...
    const userAllInfo = await Users.findByPk(id,{attributes: {exclude: ["userRole"]}}).then((response)=>{
        const emailDecrypted = CryptoJS.AES.decrypt(userAllInfo.userEmail, "MyCrypt0Key");
