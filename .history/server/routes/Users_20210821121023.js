    ...
    Bcrypt.hash(userPassword , 10)
        .then((passwordHash)=>{            
                Users.create({
                    ...
                    res.status(300).json({error : "utilisateur déja enregistré"})
                    }else if (err.errors[0].path === "userEmail"){
                        res.status(300).json({error:"email déja pris"})
                    } else if (err.errors[0].path === "userEmail" || err.errors[0].path === "userPseudo"){
                        res.status(300).json({error:"vous etes deja inscrit"})
                    }                   
                })                   
        }).catch((error)=>{
            if(error){
