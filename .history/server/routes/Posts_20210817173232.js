...
const {Posts, Likes, PostsSignalled} = require('../models');
const {validateToken} = require('../middlewares/authMiddleware')


Router.get("/",validateToken,async (req, res) => {
   const listOfPosts =await Posts.findAll({order:[['id','DESC']],include: [Likes]});
   const listOfLikeInPost = await Likes.findAll({where:{UserId:req.user.id}});
   const listOfPostInAlert = await PostsSignalled.findAll({where:{UserId:req.user.id}});
   res.json({listOfPosts: listOfPosts, listOfLikeInPost:listOfLikeInPost,listOfPostInAlert:listOfPostInAlert})
});
