...
Router.post("/login" , async (req, res)=>{
    ...
    Bcrypt.compare(userPassword, user.userPassword)
        .then((samePassword) => {
            ...
            const accesToken = sign({userPseudo: user.userPseudo, id: user.id, role:user.userRole},"keyScret");
            res.json({token: accesToken, userPseudo: userPseudo, id:user.id, role:user.userRole})
        })
})
